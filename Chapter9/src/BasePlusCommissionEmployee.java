public class BasePlusCommissionEmployee {
    ...
    public BasePlusCommissionEmployee(String firstName, String lastName, String socialSecurityNumber,
        ...
        if (grossSales < 0.0) {
            throw new IllegalArgumentException("Gross sales suppose pass 0 na!");
        }
        if (commissionRate <= 0.0 || commissionRate >= 1.0) {
            throw new IllegalArgumentException("The commission gas dey between 0 and 1 na, boss");
        }
        if (baseSalary < 0.0) {
    ...
    public String toString() {
        return String.format("%s: %s %s%n%s: %s%n%s: %.2f%n%s: %.2f%n %ss: %.2f",
